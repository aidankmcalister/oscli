...
replays the prompt flow and the emitted output from that handler, in the same
order the real handler calls them.
...
Use `answers` when you want stable prompt values. Any prompt you leave
unpinned is auto-generated by the demo without using the CLI's defaults.
Set `replay={false}` when you want the demo to run once and stop.
